...
func completeReqRespTracking(eventInfo *C.DD_ETW_EVENT_INFO, httpConnLink *HttpConnLink) {
	...
	// Time
	httpConnLink.http.ResponseLastSeen = fileTimeToUnixTime(uint64(eventInfo.event.timeStamp))
	...
	if HttpServiceLogVerbosity == HttpServiceLogVeryVerbose {
		...
		log.Infof("  Requested:      %v\n", formatUnixTime(httpConnLink.http.RequestStarted))
		log.Infof("  Responded:      %v\n", formatUnixTime(httpConnLink.http.ResponseLastSeen))
		...
		log.Infof("  Url:            %v\n", httpConnLink.url)
		log.Infof("  Method:         %v\n", httpMethodToStr(httpConnLink.http.RequestMethod))
		log.Infof("  StatusCode:     %v\n", httpConnLink.http.ResponseStatusCode)
		// <<<MORE ETW HttpService DETAILS>>>
...
func httpCallbackOnHTTPRequestTraceTaskRecvReq(eventInfo *C.DD_ETW_EVENT_INFO) {
	...
	reqRespAndLink.http.Tup = connOpen.conn.tup
	reqRespAndLink.http.RequestStarted = fileTimeToUnixTime(uint64(eventInfo.event.timeStamp))
	...
	// output details
	if HttpServiceLogVerbosity == HttpServiceLogVeryVerbose {
		log.Infof("  Time:           %v\n", formatUnixTime(reqRespAndLink.http.RequestStarted))
		log.Infof("  ActivityId:     %v\n", formatGuid(eventInfo.event.activityId))
...
func httpCallbackOnHTTPRequestTraceTaskParse(eventInfo *C.DD_ETW_EVENT_INFO) {
	...
	httpConnLink.http.RequestMethod = verbToMethod(binary.LittleEndian.Uint32(userData[8:12]))
...
func httpCallbackOnHTTPRequestTraceTaskRecvResp(eventInfo *C.DD_ETW_EVENT_INFO) {
	...
	httpConnLink.http.ResponseStatusCode = binary.LittleEndian.Uint16(userData[16:18])
...
func httpCallbackOnHTTPRequestTraceTaskSrvdFrmCache(eventInfo *C.DD_ETW_EVENT_INFO) {
	...
	if cacheEntry.cache.reqRespBound {
		...
		httpConnLink.http.ResponseStatusCode = cacheEntry.http.ResponseStatusCode
